...
/**
 * Created by Ilia Maltsev on 27.05.2015.
 */
public class SplitAnimation {
    private static AnimatorSet mSetAnim;
    private static int splitYCoord;
    private static Bitmap mBmp1=null;
    private static Bitmap mBmp2=null;
    ...
    private static void clean(Activity destActivity) {
        if (mTopImage!=null){
            mTopImage.setLayerType(View.LAYER_TYPE_NONE,null);
            try{
                destActivity.getWindowManager().removeViewImmediate(mBottomImage);
        ...
        if(mBottomImage!=null){
            ...
            try{
                destActivity.getWindowManager().removeViewImmediate(mTopImage);
            }catch (Exception ignored){
            }
        }
        mBmp1=null;
